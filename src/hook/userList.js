...
export const useUserList = ({ roomId }) => {
    ...
    const raiseHand = async (roomId, raiseHand = true) => {
        return client.publish(EMQTTEvent.RAISE_HAND + roomId, JSON.stringify({ userId: userInfo._id, raiseHand }))
    }
    const chooseWriter = async ({ roomId, userId }) => {
        return allowUserWrite({ roomId, userId })
    }
    const kick = async ({ roomId, userId }) => {
        return kickUser(roomId, userId)
    }
